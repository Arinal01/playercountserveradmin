...
const token = 'OTkxMjU2NTkxNzAzNzUyNzY1.G2oeUI.4EV26ZQYoVFI18nP24H1ZNNvza-ZrAG2NqOie0'; //Dette er discord bottens token som den skal bruge til at logge ind med
...
function activity(){ // laver funktionen activity
    setTimeout(() => { // laver et loop
        fivereborn.query("de1.sneakyhub.com","10401", (err, data) => { // starter fivereborn event, IP:PORT 147.185.221.211:47585
            if (err) { // logger hvis der er fejl
                console.log(err); // logger fejlen
            } else { //  ellers skal den sætte activity
                bot.user.setActivity(`${data.clients}/${data.maxclients} Warga | SERVER BETA | LIFE STORY ROLEPLAY`, { type: "PLAYING" }, {status: 'idle'});
            }
        });
        activity(); 
    }, 1000);
